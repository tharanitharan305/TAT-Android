...
// import '../../Widgets/Dropdowntat.dart';
...
//           currentProduct = context.read<ProductBloc>().currentProduct;
//         }
//         return Scaffold(
...
class _ProductGetterState extends State<ProductGetter> {
  ...
  void initState() {
    ...
    setCompanies();
    isUpdateMode = widget.product != null;
    currentProduct = widget.product ?? Product(
      sno: 0,
      partNo: "PartNo",
      barcode: "barcode",
      productName: "productName",
  ...
  void updateField(String field, dynamic value) {
    context.read<ProductBloc>().add(UpdateProductField(fieldName: field, value: value));
  }
  ...
  Widget buildTextField(String label, String field, String initialValue, {bool isNumeric = false}) {
    return Padding(
      ...
      child: TextFormField(
        ...
        validator: (value) => validateFields(field, value),
        onChanged: (value) => updateField(field, value),
        onSaved: (value) => updateField(field, value),
      ),
    );
  }
        ...
        return Scaffold(
          ...
          body: SingleChildScrollView(
            child: Form(
              ...
              child: Column(
                children: [
                  buildTextField("Product Code", "partNo", currentProduct.partNo!),
                  buildTextField("Barcode", "barcode", currentProduct.barcode!),
                  buildTextField("Product Name", "productName", currentProduct.productName),
                  buildTextField("Group Name", "groupName", currentProduct.groupName!),
                  buildTextField("Company", "company", currentProduct.company),
                  buildTextField("Commodity", "commodity", currentProduct.commodity),
                  buildTextField("Rack", "rack", currentProduct.rack!),
                  buildTextField("Section", "section", currentProduct.section),
                  buildTextField("Sales Tax", "salesVat", currentProduct.salesVat, isNumeric: true),
                  buildTextField("Product Tax", "godownTax", currentProduct.godownTax, isNumeric: true),
                  buildTextField("Purchase Price", "pPrice", currentProduct.pPrice.toString(), isNumeric: true),
                  buildTextField("Cost Price", "cPrice", currentProduct.cPrice.toString(), isNumeric: true),
                  buildTextField("Selling Price", "sPrice", currentProduct.sPrice.toString(), isNumeric: true),
                  buildTextField("Special Price", "splPrice", currentProduct.splPrice.toString(), isNumeric: true),
                  buildTextField("MRP", "mrp", currentProduct.mrp.toString(), isNumeric: true),
                  buildTextField("Margin", "margin", currentProduct.margin.toString(), isNumeric: true),
                  buildTextField("Stock Weight", "weight", currentProduct.weight.toString(), isNumeric: true),
                  buildTextField("Expiry Days", "expiryDays", currentProduct.expiryDays.toString(), isNumeric: true),
                  buildTextField("Supplier", "supplier", currentProduct.supplier??""),
                  buildTextField("Discount", "discound", currentProduct.discound.toString(), isNumeric: true),
                  const SizedBox(height: 20),
                  ElevatedButton(
                    onPressed: () {
                      if (key.currentState!.validate()) {
                        key.currentState!.save();
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(content: Text(isUpdateMode ? "Product Updated" : "New Product Added")),
                        );
                      }
                    },
                    child: Text(isUpdateMode ? "Update Product" : "Add Product"),
                  ),
                ],
              ),
            ),
